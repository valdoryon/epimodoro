...
const Chrono = ({ chronoWorker }) => {
  ...
  const [miliSeconds, setMiliSeconds] = useState(
    !window.localStorage.getItem('c-ml')
      ? 0
      : window.localStorage.getItem('c-ml')
  );
  const [seconds, setSeconds] = useState(
    !window.localStorage.getItem('c-s') ? 0 : window.localStorage.getItem('c-s')
  );
  const [minutes, setMinutes] = useState(
    !window.localStorage.getItem('c-m') ? 0 : window.localStorage.getItem('c-m')
  );
  const [hours, setHours] = useState(
    !window.localStorage.getItem('c-h') ? 0 : window.localStorage.getItem('c-h')
  );
  const [isRunning, setIsRunning] = useState(
    !window.localStorage.getItem('c-r?')
      ? false
      : window.localStorage.getItem('c-r?') === 'true'
  );
        ...
        <div className='chrono-main_container'>
          <div className='chrono-container'>
            <div className='chrono-text_container'>
              ...
              <span className='chrono-text'>.</span>
              <span className='chrono-number milli'>
                {(miliSeconds < 10 ? '0' : '') + miliSeconds}
              </span>
            </div>
            <div className='chrono-buttons_container'>
              <button onClick={handleStartClick} className='timer-button'>
                {isRunning ? 'Parar' : 'Iniciar'}
              </button>
              <button onClick={handleResetClick} className='timer-button'>
                Reiniciar
              </button>
            </div>
          </div>
