...
const Timer = ({ timerWorker }) => {
  ...
  const [seconds, setSeconds] = useState(
    !window.localStorage.getItem('s') ? 0 : window.localStorage.getItem('s')
  );
  const [minutes, setMinutes] = useState(
    !window.localStorage.getItem('m') ? 25 : window.localStorage.getItem('m')
  );
  const [hours, setHours] = useState(
    !window.localStorage.getItem('h') ? 0 : window.localStorage.getItem('h')
  );
  const [isRunning, setIsRunning] = useState(
    !window.localStorage.getItem('r?')
      ? false
      : window.localStorage.getItem('r?') === 'true'
  );
  ...
  return (
    <>
      ...
      <section className='timer-wrapper'>
        <div className='timer-main_container'>
          <div className='timer-container'>
            <div className='timer-text'>
              ...
              <span className='timer-num_text'>:</span>
        ...
        {isShown && (
          <EditTimer
            handleAtZeroState={handleAtZeroState}
            handleIsShownState={handleModifyClick}
